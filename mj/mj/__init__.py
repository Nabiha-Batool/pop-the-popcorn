...
#from controllers.controllers import InfoCRUDController
...
def main(global_config, **settings):
    ...
    config.add_tween('mj.auth.authenticator')
    ...
    config.add_route('login', '/user_login')
    ...
    #add_crud_handler(config, 'info', '/info_crud', InfoCRUDController)

    add_admin_handler(config, DBSession, get_models(mj), 'admin.', '/admin', AdminController)
...
def configure_app_routes(config):
    ...
    for app_name in enabled_apps:
        ...
        app_module = importlib.import_module(".apps.%s" % app_name, "mj")
